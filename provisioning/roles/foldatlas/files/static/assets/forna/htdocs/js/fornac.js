function FornaContainer(h, f) {
	...
	function b(b, d, e) {
		return b.hasOwnProperty(d.num) ? (val = parseFloat(b[d.num]), isNaN(val) ? b[d.num] : e(val)) : "white"
	}
	...
	function g(b) {
		...
		return function(d, I, l, k, h) {
			if (d.point && d.point !== b) {
				...
				q < p && (q = (q - p) / q * .1, b.x -= m *= q, b.y -= w *= q, d.point.x += m, d.point.y += w)
			}
	...
	if (1 < arguments.length)
		for (var w in f) d.options.hasOwnProperty(w) &&
			(d.options[w] = f[w]);
	...
	d.transitionRNA = function(b, e, g) {
		...
		b.transition().attr("transform", function(b) {
			console.log("d after", b);
	...
	d.recalculateGraph = function(b) {
		...
		for (f = 0; f < d.extraLinks.length; f++) {
			...
			if ("intermolecule" == d.extraLinks[f].link_type)
				for (fake_links = d.graph.links.filter(function(b) {
						return (b.source == d.extraLinks[f].source || b.source == d.extraLinks[f].target || b.target == d.extraLinks[f].source || b.target == d.extraLinks[f].source) && "fake" == b.link_type
					}), b = 0; b < fake_links.length; b++) e = d.graph.links.indexOf(fake_links[b]), d.graph.links.splice(e, 1);
	...
	d.addNodes = function(b) {
		...
		0 < d.graph.nodes.length ? (max_x = d3.max(d.graph.nodes.map(function(b) {
			return b.x
		})), max_y = d3.max(d.graph.nodes.map(function(b) {
			return b.y
		}))) : max_y = max_x = 0;
		b.nodes.forEach(function(b) {
	...
	// $.get(location.origin + window.location.pathname.replace(/[^\/]+$/g, "") + "css/fornac.css", function(b) {
	// 	t.text(b.replace(/[\s\n]/g, ""))
	// });
	d.options.svg = u;
	var x = u.append("svg:g").on("mousemove",
		function() {
			m && (mpos = d3.mouse(v.node()), H.attr("x1", m.x).attr("y1", m.y).attr("x2", mpos[0]).attr("y2", mpos[1]))
	...
	d.brusher = d3.svg.brush().x(D).y(E).on("brushstart", function(b) {
		p.selectAll("g.gnode").selectAll(".outline_node").each(function(b) {
	...
	d.center_view = function() {
		0 !== d.graph.nodes.length && (min_x = d3.min(d.graph.nodes.map(function(b) {
				return b.x
			})), min_y = d3.min(d.graph.nodes.map(function(b) {
				return b.y
			})), max_x = d3.max(d.graph.nodes.map(function(b) {
				return b.x
			})), max_y = d3.max(d.graph.nodes.map(function(b) {
				return b.y
			})),
	...
	d.force = d3.layout.force().charge(function(b) {
		...
		return b.link_type in d.linkStrengths ? d.linkStrengths[b.link_type] : d.linkStrengths.other
	}).gravity(0).nodes(d.graph.nodes).links(d.graph.links).chargeDistance(110).size([d.options.svgW, d.options.svgH]);
	var H = v.append("line").attr("class", "drag_line").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", 0),
		C = !1,
		B = !1;
	...
	var G = d3.behavior.drag().on("dragstart", function(b) {
		...
		b.selected || B || p.selectAll("g.gnode").selectAll(".outline_node").classed("selected", function(b) {
	...
	d.add_link = function(b) {
		b.source.rna == b.target.rna ? (r = b.source.rna, r.pairtable[b.source.num] = b.target.num, r.pairtable[b.target.num] = b.source.num, update_rna_graph(r)) : (b.link_type = "intermolecule", d.extraLinks.push(b));
		d.recalculateGraph();
		d.update()
	};
			...
			if (q = b, q == m) q = m = null;
				...
				for (i = 0; i < d.graph.links.length; i++) {
					if (d.graph.links[i].source == m || d.graph.links[i].target == m || d.graph.links[i].source == q || d.graph.links[i].target == q)
						if ("basepair" == d.graph.links[i].link_type || "pseudoknot" == d.graph.links[i].link_type) return;
					if ((d.graph.links[i].source ==
							q && d.graph.links[i].target == m || d.graph.links[i].source == m && d.graph.links[i].target == q) && "backbone" == d.graph.links[i].link_type) return
				}
				"middle" != q.node_type && "middle" != m.node_type && "label" != q.node_type && "label" != m.node_type && d.add_link(b)
			}
	};
	...
	d.update = function() {
		...
		link_lines.classed("link", !0).attr("x1", function(b) {
			return b.source.x
		}).attr("y1",
			function(b) {
				return b.source.y
		...
		// apply inline styles
		z.selectAll("[link_type=backbone],[link_type=label_link]").style({
			"stroke": "#999",
			"stroke-opacity": "0.8",
			"stroke-width": "2"
		});
		z.selectAll("[link_type=basepair]").style({
			"stroke": "red",
			"stroke-opacity": "0.8",
			"stroke-width": "2"
		});
		...
		gnodes_enter = e.enter().append("g").classed("noselect", !0).classed("gnode", !0).attr("struct_name", function(b) {
		...
		d.force.on("tick", function() {
			for (var b = d3.geom.quadtree(real_nodes), d = 0, f = real_nodes.length; ++d < f;) b.visit(g(real_nodes[d]));
		...
		for (var b = "", e = 1, g = {
			...
			if (">" == h[l][0]) b = h[l].trim().slice(1),
				...
				for (var d = 0; d < words.length; d++)
					if (isNaN(words[d]))
						if (0 === words[d].search("range")) parts = words[d].split("="), parts_right = parts[1].split(":"), g.range = [parts_right[0], parts_right[1]];
					...
					color = parts[1];
					for (var w = 0; w < nums.length; w++) isNaN(color) ? g.color_values[b][nums[w]] =
	...
	f.normalizeColors = function() {
		...
		for (b in f.colors_json) {
			...
			for (n in f.colors_json.color_values[b]) h = f.colors_json.color_values[b][n],
				"number" == typeof h && (f.colors_json.color_values[b][n] = (h - e) / (g - e))
		}
		return f
	};
...
function RNAGraph(h, f, k) {
	...
	b.nucs_to_nodes = {};
	b.addUids = function(e) {
		for (var g = 0; g < e.length; g++) b.nodes[g].uid =
			e[g];
		return b
	};
	...
	b.addPositions = function(e, g) {
		...
		for (var f = 0; f < label_nodes.length; f++) label_nodes[f].x = g[f][0], label_nodes[f].px = g[f][0], label_nodes[f].y = g[f][1], label_nodes[f].py = g[f][1];
		return b
	};
	b.get_positions = function(e) {
		...
		for (var g = 0; g < nucleotide_nodes.length; g++) positions.push([nucleotide_nodes[g].x,
	...
	b.addFakeNode = function(e) {
		...
		for (j = 0; j < e.length; j++) 0 === e[j] || e[j] > b.dotbracket.length || (b.links.push({
			...
			link_type: "fake",
			value: c
		}), from_node = b.nodes[e[j] - 1], "x" in from_node && (new_x += from_node.x, new_y += from_node.y, coords_counted += 1));
	...
	b.elementsToJson = function() {
		...
		for (var e = 0; e < b.elements.length; e++)
			for (nucs = b.elements[e][2], j = 0; j <
				nucs.length; j++) elem_types[nucs[j]] = b.elements[e][0];
	...
	b.pt_to_elements = function(e, g, f, h) {
		...
		e[f] === h && (k.push(f), m.push(h), combined = k.concat(m), 4 < combined.length && (0 === g ? d.push(["e", g, k.concat(m).sort(number_sort)]) : d.push(["i", g, k.concat(m).sort(number_sort)])));
	...
	b.recalculateElements = function() {
		...
		if (b.circular && (external_loop = b.elements.filter(function(b) {
				if ("e" == b[0]) return !0
			...
			hloop = !0;
			num_greater = 0;
			for (var e = 1; e < nucs.length; e++) 1 < nucs[e] - prev && (num_greater += 1), prev = nucs[e];
			eloop[0] = 1 == num_greater ? "h" : 2 == num_greater ? "i" : "m"
		}
		return b
	};
	b.removePseudoknots = function() {
		b.pseudoknotPairs = 1 < b.pairtable.length ? rnaUtilities.removePseudoknotsFromPairtable(b.pairtable) : [];
		return b
	};
...
molecules_to_json = function(h) {
	...
	for (e = 0; e < h.extra_links.length; e++) link = h.extra_links[e], link.source =
...
simple_xy_coordinates = function(h) {
	...
	poss = [];
	poss.push([f[0], k[0]]);
	for (h = 1; h < b; h++) f[h] = f[h - 1] + 15 * Math.cos(e),
