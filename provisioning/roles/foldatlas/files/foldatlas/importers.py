...
class SequenceImporter():
    ...
    def execute( self ):
        ...
        for strain in settings.strains:
            ...
            n_strains += 1
            if self.strain_limit and n_strains >= self.strain_limit:
                break
        db_session.commit()
    ...
    def execute_genes( self, strain_config ):
            ...
            for gff_line in gff_file:
                ...
                if feature_type == "gene":
                    ...
                    if len( feature_rows ) > 0:  # this is needed to stop it going wrong at the beginning
                        ...
                        if self.gene_limit and n_genes_added[ chr_id ] >= self.gene_limit:
        ...
        if len( feature_rows ) > 0 and \
                (self.gene_limit is None or n_genes_added[ chr_id ] < self.gene_limit):
            self.execute_gene( feature_rows, strain_config[ "name" ] )
            n_genes_added[ chr_id ] += 1
    ...
    def execute_gene( self, feature_rows, strain_id ):
        ...
        for feature_row in feature_rows:  # Loop through annotation rows in the gff file, all related to the current gene
            ...
            if new_gene_id:
                ...
                if self.filter_genes and new_gene_id not in self.filter_genes:
            ...
            elif gene_id:  # Handle transcript entries - if the gene is legit
                transcript_id = self.find_attribs_value( "ID=Transcript", attribs )
                if transcript_id:  # it's a transcript entry
                    ...
                    if transcript_id:
                        # it's a transcript feature entry

                        # put a filter here? some elements are not worth storing?

                        self.features_to_write.append( Feature( transcript_id=transcript_id,
        ...
        while True:
            if candidate_transcript_id in self.transcript_ids_seen_this_strain:
                ...
                if candidate_transcript_id != transcript_id:
                    print( "Transcript ID [{}] was a duplicate, renamed to [{}]".format( transcript_id,
...
# Commented out by HW on 2017-10-09 becuase foldatalas does not currently show alignments
#
    ...
    def execute( self, experiment_config ):
        ...
        with open( coverage_filepath ) as coverage_file:
            for coverage_line in coverage_file:
                ...
                nt_measurement_run_id = experiment_config[ "nucleotide_measurement_run_id" ]

                update_q = update( NucleotideMeasurementSet ) \
                    .where( and_( NucleotideMeasurementSet.nucleotide_measurement_run_id == nt_measurement_run_id,
...
class ReactivitiesImporter():
    def execute( self, experiment_config ):
        ...
        with open( experiment_config[ "nucleotides_filepath" ], "r" ) as input_file:
            while True:
                ...
                if normalised_reactivities is None:
                    continue
    ...
    def norm_2_8( self, transcript_id, seq, plus_counts, minus_counts ):
        ...
        for pos in range( 0, len( scaled_log_plus ) ):
            if scaled_log_plus[ pos ] is None:
    ...
    def log_counts( self, counts ):
        ...
        for count in counts:
            if count is None:
                ...
                out.append( math.log( float( count + 1 ) ) )
        return out
    ...
    def scale_by_2_8( self, minus_subbed ):
        ...
        for value in minus_subbed:
            if value and value > 0:  # only consider values > 0
        ...
        # Generate 2-8% list
        idx1 = int( round( len( norm_values ) * 0.02 ) )
        idx2 = int( round( len( norm_values ) * 0.08 ) )
        v8 = norm_values[ idx1:idx2 + 1 ]
        ...
        out = [ ]
        for i in range( len( minus_subbed ) ):
...
# Commented out by HW on 2017-10-09 because of the comment below and 'settings.coverage_filepath' does not exist
#
...
class StructureTidsExporter():
    ...
    def export( self ):
        results = engine.execute( "SELECT DISTINCT transcript_id FROM structure" )
...
class MinusPlusCompiler():
    ...
    def run( self ):
        ...
        results = engine.execute( "SELECT DISTINCT id FROM transcript ORDER BY id" )

        tids = [ row[ 'id' ] for row in results ]
