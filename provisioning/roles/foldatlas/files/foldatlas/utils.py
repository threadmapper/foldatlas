...
class FastaSplitter():
    """
    Split big fasta file into smaller ones.
    This is for doing HPC structure predictions
    """

    def __init__( self ):
        self.n_chunks = 256
        self.source_filepath = settings.data_folder + "/transcripts.fasta"
    ...
    def split( self ):
        ...
        n_seqs = 0
        handle = open( self.source_filepath, "r" )
...
def grab_structures():
    ...
    source_folder = "/var/www/raw_structures_folder"
...
def build_dot_bracket( positions ):
    ...
    if dot_bracket_str.count( '(' ) != dot_bracket_str.count( ')' ):
...
# 			if gene in filename:
# 				shutil.copyfile(source_folder+"/"+filename, dest_folder+"/"+filename)
